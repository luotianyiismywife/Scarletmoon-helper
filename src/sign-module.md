...
- 真实的每日奖励签到动作是 **`kf_growup.php?ok=3&safeid=...`**（2026-08-03/08-04 连续两天未签到状态下实测验证成功）。`ok` 参数疑似为操作类型：`ok=2`=ID 颜色设置、`ok=3`=每日奖励领取。
...
- 过滤规则：
  ...
  - 其次匹配 `kf_growup.php?ok=3`（每日奖励领取动作，已验证）
- 如果 DOM 查找失败，则退而使用正则匹配（同样排除 `ok=2`）

返回值：有效签到动作链接字符串，如 `kf_growup.php?ok=3&safeid=<safeid>`
...
- `log(...args)`：统一调试日志，输出 `[绯月签到助手]` 前缀（v0.6.0 起）
- `logError(...args)`：统一错误日志，同上前缀，走 `console.error`
